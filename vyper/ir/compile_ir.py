        ...
        if self[val].uses_left == 0:
            ...
            for k in self.keys():
                if self[k].height == 0:
                    ...
                    assert to_swap is None
                    to_swap = self[k].height + n_temps
                self[k].height -= 1

            assert to_swap is not None, (val, k, self)
...
def compile_to_assembly(code, no_optimize=False):
    ...
    res = _compile_to_assembly(code, CompileState())
...
def _compile_to_assembly(code, compile_state):
    ...
    def _compile(code):
        ret = _compile_to_assembly(code, compile_state)
        compile_state.height += code.valency
        return ret
    ...
    if isinstance(code.value, str) and code.value.upper() in get_opcodes():
        ...
        compile_state.height -= len(code.args)
        return o
    ...
    # Variables connected to with statements
        ...
        return compile_state.variables.use_var(code.value, compile_state.height)
