...
class CompilerData:
    ...
    def bytecode(self) -> bytes:
        ...
        if self.settings.experimental_eof:
            return generate_EOFv1(self.assembly, compiler_metadata=metadata)
        else:
    ...
    def bytecode_runtime(self) -> bytes:
        if self.settings.experimental_eof:
            return generate_EOFv1(self.assembly_runtime, compiler_metadata=None)
        else:
...
def generate_EOFv1(assembly: list, compiler_metadata: Optional[Any]) -> bytes:
    bytecode, _ = compile_ir.assembly_to_evm(
        assembly, emit_headers=True, compiler_metadata=compiler_metadata
    )
