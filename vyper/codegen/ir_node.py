...
class IRnode:
    ...
    # anything that can produce reentrancy (including read-only reentrancy)
    @cached_property
    def contains_risky_call_rw(self):
        ret = self.value in ("create", "create2")
        ret |= self.value in ("call", "delegatecall", "staticcall")

        for arg in self.args:
            ret |= arg.contains_risky_call_rw

        if getattr(self, "is_self_call", False):
            ret |= self.invoked_function_ir.func_ir.contains_risky_call_rw

        return ret

    # anything that can produce reentrancy (excluding read-only reentrancy)
    @cached_property
    def contains_risky_call_wo(self):
        ret = self.value in ("create", "create2")
        ret |= self.value in ("call", "delegatecall")

        for arg in self.args:
            ret |= arg.contains_risky_call_wo

        if getattr(self, "is_self_call", False):
            ret |= self.invoked_function_ir.func_ir.contains_risky_call_wo
